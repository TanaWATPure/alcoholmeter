            ...
            function updateAlcoholTable() {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.length > 0) {
                            alcoholTable.clear().draw(); // Clear existing data and redraw

                            // Loop through each row of data received
                            $.each(data, function (index, row) {
                                var rowClass = (parseFloat(row.alcoholvalue) > 20) ? 'high-alcohol' : 'low-alcohol'; // Determine row class

                                // Create the "Edit" button HTML
                                var editButton = '<button type="button" class="btn btn-primary btn-sm edit-alcohol-btn" ' +
                                                    'data-bs-toggle="modal" ' +
                                                    'data-bs-target="#editAlcoholModal" ' +
                                                    'data-alcohol-id="' + row.id + '" ' +
                                                    'data-rfid="' + row.rfid + '" ' +
                                                    'data-alcoholvalue="' + row.alcoholvalue + '" ' +
                                                    'data-name="' + row.name + '" ' +
                                ...
                                // Create the "Delete" form HTML
                                var deleteForm = '<form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" style="display:inline-block;">' +
                                                    '<input type="hidden" name="alcohol_id" value="' + row.id + '">' +
                                                    '<button type="submit" name="delete_alcohol" class="btn btn-danger btn-sm">' +
                                                        '<i class="fas fa-trash-alt"></i> ลบ' +
                                                    '</button>' +
                                                '</form>';
                                ...
                                alcoholTable.row.add([
                                    ...
                                    row.created_at,
                                    row.updated_at,
                                    editButton + deleteForm  // Add the buttons to the last column
                                ]).node().className = 'alcohol-row ' + rowClass; // Set the row class

                            }); 

                            alcoholTable.draw(); // Redraw the DataTable to display the changes
                        } else {
                            // Handle the case where no data is returned (e.g., display a message)
                            alcoholTable.clear().draw();
                            alcoholTable.row.add(['No data available', '', '', '', '', '', '', '', '']).draw();
                        }
                    },
                    error: function () {
                        console.error('Error fetching alcohol data.');
                        // Consider adding a visual indication to the user that the data could not be loaded. 
                    }
                });
            }

            updateAlcoholTable(); // Initial call to load data
            setInterval(updateAlcoholTable, 5000); // Update data every 5 seconds
        });
    </script>
</body>
</html>